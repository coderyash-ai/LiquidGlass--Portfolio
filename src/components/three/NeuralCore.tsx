...
function CoreSystem({
  ...
  const particleData = useMemo(() => {
    ...
    const torusCyan = new Color('#00B8D9')
    const primaryLiquid = new Color('#00D4E8')
    const secondaryLiquid = new Color('#008FA8')
    const highlightCyan = new Color('#67F3FF')
    const toxicGreen = new Color('#39FF88')
  ...
  return (
    <div className="neural-core" aria-hidden="true">
      <Canvas dpr={[1, 1.6]} camera={{ position: [0, 0, 5.2], fov: 35 }} gl={{ antialias: true, alpha: true }}>
        ...
        <pointLight position={[-4, -2, 3]} intensity={14} color="#00B8D9" />
        <pointLight position={[4, 3, -2]} intensity={8} color="#39FF88" />
        <Suspense fallback={null}>
          <CoreSystem reduced={reduced} onClick={handleBurst} />
          {!reduced ? <AmbientField /> : null}
          <Environment preset="city" />
        </Suspense>
        <ContactShadows position={[0, -1.85, 0]} opacity={0.45} scale={8} blur={2.6} far={3.2} color="#080D16" />
      </Canvas>
